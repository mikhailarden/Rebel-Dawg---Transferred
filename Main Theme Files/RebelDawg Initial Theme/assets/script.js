 ...
 * https://modernizr.com/download/?-backgroundblendmode-cssanimations-csscalc-csscolumns-cssgradients-csspointerevents-csstransforms-csstransforms3d-csstransitions-flexbox-flexboxtweener-opacity-placeholder-pointerevents-rgba-touchevents-mq-setclasses !*/
...
const inputValidationCharmColor = (className,name) => {
     const addToCartButton = $('.product__add-to-cart');
//   console.log('im hungry color');
  if($('.hulkapps_option_child input:checked').attr("name", "properties[Mini Charm Color]").length === 0) {	
    
    	addToCartButton.attr('disabled', 'disabled')
        .removeClass('button--primary button--loading')
        .addClass('button--secondary')
    .text("Whoops! Some fields are missing from your tag order");
  }


     if($('.hulkapps_option_child input:checked').attr("name", "properties[Mini Charm Color]").length >= 1 && ($('.rebel__personalizer').css('display') == 'block')) {
       $('.hulkapps_option_child input').attr("name", name).change(function() {
  		addToCartButton.removeAttr('disabled').removeClass('button--loading button--secondary')
...
// const inputValidationCharmType = (className,name) => {
//      const addToCartButton = $('.product__add-to-cart');
//   console.log('im hungry');
//   if($('.hulkapps_option_child input:checked').attr("name", "properties[Mini Charm Style]").length === 0) {	
//     	addToCartButton.attr('disabled', 'disabled')
//         .removeClass('button--primary button--loading')
//         .addClass('button--secondary')
//     .text("Whoops! Some fields are missing from your tag order");
...
//      if($('.hulkapps_option_child input:checked').attr("name", "properties[Mini Charm Style]").length >= 1 && ($('.rebel__personalizer').css('display') == 'block')) {
//        $('.hulkapps_option_child input').attr("name", name).change(function() {
//   		addToCartButton.removeAttr('disabled').removeClass('button--loading button--secondary')
//         .addClass('button--primary')
//         .text(window.languages.productFormAddToCart);
...
// var data = [
//     {a: 'properties[Tag Size]'},
//     {a: 'properties[Mini Charm Style]'},
//     {a: 'properties[Mini Charm Color]'},
//     {a: 'properties[Tag Sizes]'},
//     {a: 'properties[Mini Charm Styles]'},
//     {a: 'properties[Mini Charm Colors]'}
// ];
...
//     inputValidation(radioOption,entry.a);
//     inputValidation(radioOption,entry.b);
// });
...
let miniCharmStyle="properties[Mini Charm Style]";
let miniCharmColor = "properties[Mini Charm Color]";
 let fontType = "properties[Text Style]";
var radioOption = $('.hulkapps_option_child input:checked');
 var swatchOption = $('.hulkapps_swatch_option div input:checked')
  	var tagName = "properties[Tag Size]";
  
  $('.rebel__personalizer').change(function() {
//     inputValidationCharmType(miniCharmStyle);
//      inputValidationCharmColor(miniCharmColor);
    
//       data.forEach(function(entry) {
//     inputValidation($('.hulkapps_option_child input:checked'),entry.a);
//     inputValidation($('.hulkapps_option_child input:checked'),entry.b);
// });    
  });
...
(function ($) {
  ...
  Plugin.prototype._removeLine = function(event) {
    ...
    && typeof BOLD.common.cartDoctor.fix === 'function'){
      newCart = BOLD.common.cartDoctor.fix(newCart);
    }
...
(function ($) {
   ...
   */
   ...
   this.addToCartButton.attr('disabled', 'disabled').addClass('button--loading');
  	 ...
  	 var attr = $(".hulkapps_radio_option").children("input").attr('name', tagName);
      if($(".hulkapps_radio_option").children("input:checked").attr('name', tagName).length === 0 &&  attr.length >= 1) {
		console.log('please pick a size');
         
    this.addToCartButton.attr('disabled', 'disabled')
        .removeClass('button--primary button--loading')
        .addClass('button--secondary')
        .text("Woops! You need to pick a tag size");
	    
        setTimeout(function() {
        self.addToCartButton.removeAttr('disabled').removeClass('button--loading button--secondary')
        .addClass('button--primary')
        .text(window.languages.productFormAddToCart);
   ...
   }).then(pbHandleCart).then(function() {
     $.ajax({
       method: 'GET',
       url: '/cart.js',
       dataType: 'json'
       ...
       if(typeof BOLD === 'object' && BOLD.common
         ...
         cart = BOLD.common.cartDoctor.fix(cart);
          ...
          console.log("is this running from ajax add to cart");
      
       // Last parameter set to true allow to indicate the cart to force open the mini-cart)
//        $(document).trigger('theme:cart:updated', [cart, false, false]);
//        self.addToCartButton.addAttr('disabled').addClass('button--loading');
  ...
  (function() {
    var initProducts = function(domElement) {
      ...
      domElement.slick({
        ...
        swipeToSlide: true,
        initialSlide: 0 ,
        prevArrow: '<button type="button" class="slick-prev"><svg class="arrow-left" enable-background="new 0 0 512 512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="m491.318 235.318h-470.636c-11.422 0-20.682 9.26-20.682 20.682 0 11.423 9.26 20.682 20.682 20.682h470.636c11.423 0 20.682-9.259 20.682-20.682 0-11.422-9.259-20.682-20.682-20.682z"/><path d="m49.932 256 161.863-161.864c8.077-8.077 8.077-21.172 0-29.249-8.077-8.076-21.172-8.076-29.249 0l-176.488 176.488c-8.077 8.077-8.077 21.172 0 29.249l176.488 176.488c4.038 4.039 9.332 6.058 14.625 6.058s10.587-2.019 14.625-6.058c8.077-8.077 8.077-21.172 0-29.249z"/></svg></button>',
        nextArrow: '<button type="button" class="slick-next"><svg class="arrow-right" enable-background="new 0 0 31.49 31.49" viewBox="0 0 31.49 31.49" xmlns="http://www.w3.org/2000/svg"><path d="m21.205 5.007c-.429-.444-1.143-.444-1.587 0-.429.429-.429 1.143 0 1.571l8.047 8.047h-26.554c-.619.001-1.111.493-1.111 1.112s.492 1.127 1.111 1.127h26.554l-8.047 8.032c-.429.444-.429 1.159 0 1.587.444.444 1.159.444 1.587 0l9.952-9.952c.444-.429.444-1.143 0-1.571z" fill="#1e201d"/></svg></button>',
        responsive: [
            {
            breakpoint: 600,
            settings: {
    ...
    $(window).on('shopify:section:unload', '.shopify-section__cart-related-products', function(event) {
  ...
  (function() {
    var initProducts = function(domElement) {
      ...
      domElement.slick({
        ...
        speed: 400,
        swipeToSlide: true,
        initialSlide: Math.ceil(domElement.attr('data-slides-count') / 2) ,
        prevArrow: '<button type="button" class="slick-prev"><svg class="arrow-left" enable-background="new 0 0 512 512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="m491.318 235.318h-470.636c-11.422 0-20.682 9.26-20.682 20.682 0 11.423 9.26 20.682 20.682 20.682h470.636c11.423 0 20.682-9.259 20.682-20.682 0-11.422-9.259-20.682-20.682-20.682z"/><path d="m49.932 256 161.863-161.864c8.077-8.077 8.077-21.172 0-29.249-8.077-8.076-21.172-8.076-29.249 0l-176.488 176.488c-8.077 8.077-8.077 21.172 0 29.249l176.488 176.488c4.038 4.039 9.332 6.058 14.625 6.058s10.587-2.019 14.625-6.058c8.077-8.077 8.077-21.172 0-29.249z"/></svg></button>',
        nextArrow: '<button type="button" class="slick-next"><svg class="arrow-right" enable-background="new 0 0 31.49 31.49" viewBox="0 0 31.49 31.49" xmlns="http://www.w3.org/2000/svg"><path d="m21.205 5.007c-.429-.444-1.143-.444-1.587 0-.429.429-.429 1.143 0 1.571l8.047 8.047h-26.554c-.619.001-1.111.493-1.111 1.112s.492 1.127 1.111 1.127h26.554l-8.047 8.032c-.429.444-.429 1.159 0 1.587.444.444 1.159.444 1.587 0l9.952-9.952c.444-.429.444-1.143 0-1.571z" fill="#1e201d"/></svg></button>',
        responsive: [
            {
    ...
    $(window).on('shopify:section:unload', '.shopify-section__featured-collection', function(event) {
      $(event.target).find('.list-products').slick('unslick');
    });
